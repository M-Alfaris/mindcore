...
if TYPE_CHECKING:
    from mindcore.flr.recall import FLR, Memory
...
class UsageDetectionResult:
    ...
    def __post_init__(self):
        ...
        if self.used_memories:
            self.avg_usage_confidence = sum(m.usage_confidence for m in self.used_memories) / len(
                self.used_memories
            )
...
class UsageDetector:
    ...
    def _analyze_memory_usage(
        ...
        if memory.topics and topic_matches > 0:
            ...
            confidence_components.append(("topic", min(1.0, topic_score) * self.topic_match_weight))
        ...
        return {
            ...
            "topic_stats": {
                t: {
                    "used": s["used"],
                    "total": s["total"],
                    "rate": s["used"] / s["total"] if s["total"] > 0 else 0,
                }
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_usage_rate": sum(r.usage_rate for r in self._usage_history)
