...
class MetadataSignal:
    ...
    def __post_init__(self):
        ...
        if not self.matched_topics:
            self.matched_topics = list(set(self.assigned_topics) & set(self.query_topics))
...
class MetadataFeedbackTracker:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            "topic_effectiveness": {k: v.to_dict() for k, v in self._topic_effectiveness.items()},
            "category_effectiveness": {
                k: v.to_dict() for k, v in self._category_effectiveness.items()
            },
            "intent_effectiveness": {k: v.to_dict() for k, v in self._intent_effectiveness.items()},
            "type_effectiveness": {k: v.to_dict() for k, v in self._type_effectiveness.items()},
