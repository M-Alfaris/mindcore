...
class QueryOptimizer:
    ...
    def _prune_old_data(self) -> None:
        ...
        self._usage_history = [(t, r) for t, r in self._usage_history if t > cutoff]
    ...
    def optimize_query(
                ...
                if category in self._category_stats:
                    ...
                    if stats.times_retrieved >= 3:
                        category_scores.append(
                            (category, stats.effectiveness_score, stats.usage_rate)
                        )
                    else:
            ...
            for category, score, usage_rate in category_scores:
                if usage_rate < self.removal_threshold and len(optimized_categories) >= 1:
                    ...
                    reasoning_parts.append(
                        f"Removed category '{category}' (usage rate: {usage_rate:.0%})"
                    )
                else:
                    ...
                    if usage_rate >= self.boost_threshold:
                        ...
                        reasoning_parts.append(
                            f"Boosted category '{category}' (usage rate: {usage_rate:.0%})"
                        )
        ...
        if self.enable_limit_adjustment and self._usage_history:
            ...
            if avg_usage_rate < LOW_USAGE_THRESHOLD:
                ...
                optimized_limit = max(
                    MIN_RETRIEVAL_LIMIT, int(original_limit * LOW_USAGE_LIMIT_MULTIPLIER)
                )
                reasoning_parts.append(
                    f"Reduced limit to {optimized_limit} (low usage rate: {avg_usage_rate:.0%})"
                )
            elif avg_usage_rate > HIGH_USAGE_THRESHOLD:
                # High usage - can increase limit
                optimized_limit = min(
                    MAX_RETRIEVAL_LIMIT, int(original_limit * HIGH_USAGE_LIMIT_MULTIPLIER)
                )
                reasoning_parts.append(
                    f"Increased limit to {optimized_limit} (high usage rate: {avg_usage_rate:.0%})"
                )

        # Calculate confidence
        confidence = min(
            1.0, total_samples / CONFIDENCE_SAMPLE_THRESHOLD
        ...
        return QueryOptimization(
            ...
            if reasoning_parts
            else "No significant optimizations",
        )
    ...
    def _generate_recommendations(
        ...
        if bottom_topics:
            ...
            if poor:
                recs.append(
                    f"Consider removing these topics from SVL or refining their definitions: {', '.join(poor)}"
                )

        if top_topics:
            recs.append(
                f"Best performing topics to prioritize: {', '.join([t for t, _ in top_topics[:3]])}"
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "overall_usage_rate": self._total_used / self._total_retrieved
