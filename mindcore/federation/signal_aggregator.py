...
class CrossAgentSignalAggregator:
    ...
    def _compute_aggregation(
        ...
        if self.trust_policy == TrustPolicy.NAMESPACE_WEIGHTED:
            return self._aggregate_namespace_weighted(aggregated, reference_scope)
        if self.trust_policy == TrustPolicy.REPUTATION_BASED:
            return self._aggregate_reputation_based(aggregated)
        if self.trust_policy == TrustPolicy.RECENCY_WEIGHTED:
            return self._aggregate_recency_weighted(aggregated)
        if self.trust_policy == TrustPolicy.HIERARCHICAL:
    ...
    def _aggregate_namespace_weighted(
        ...
        for signal in aggregated.signals.values():
            ...
            if reference_scope:
                # Bonus for same team
                if reference_scope.team and signal.agent_scope.team == reference_scope.team:
                    weight += self.weight_config.same_team_bonus

                # Bonus for same department
                elif (
                    reference_scope.department
                    and signal.agent_scope.department == reference_scope.department
                ):
                    weight += self.weight_config.same_department_bonus

                # Bonus for same agent type
                if (
                    reference_scope.agent_type
                    and signal.agent_scope.agent_type == reference_scope.agent_type
                ):
