...
class AccessScope:
    ...
    def matches_level(self, level: AccessLevel, target: AccessScope) -> bool:
        ...
        if level == AccessLevel.DEPARTMENT:
            return self.org_id == target.org_id and self.department == target.department
        ...
        if level == AccessLevel.AGENT_TYPE:
            return self.org_id == target.org_id and self.agent_type == target.agent_type
...
class AccessPolicy:
    ...
    def is_available(self) -> bool:
        ...
        available = (
            self.available_from
            if self.available_from.tzinfo
            else self.available_from.replace(tzinfo=timezone.utc)
        )
...
class MemoryACL:
    ...
    def from_dict(cls, data: dict[str, Any]) -> MemoryACL:
        ...
        policy = AccessPolicy(
            ...
            expires_at=datetime.fromisoformat(policy_data["expires_at"])
            if policy_data.get("expires_at")
            else None,
            available_from=datetime.fromisoformat(policy_data["available_from"])
            if policy_data.get("available_from")
            else None,
