...
class TrivialMessageDetector:
    ...
    PATTERNS: List[Tuple[str, TrivialCategory, float]] = [
        ...
        # Farewells
        (r"^(bye|goodbye|cya|see\s+you|later|take\s+care|cheers)[\s!.,?]*$", TrivialCategory.FAREWELL, 0.1),
        (r"^(good\s*night|gn|ttyl|talk\s+later)[\s!.,?]*$", TrivialCategory.FAREWELL, 0.1),

        # Confirmations (low importance)
        (r"^(yes|yeah|yep|yup|yea|aye|affirmative|correct|right|exactly)[\s!.,?]*$", TrivialCategory.CONFIRMATION, 0.15),
        (r"^(no|nope|nah|negative|wrong|incorrect)[\s!.,?]*$", TrivialCategory.CONFIRMATION, 0.15),
        (r"^(ok|okay|k|kk|alright|all\s*right|sure|fine|got\s+it)[\s!.,?]*$", TrivialCategory.CONFIRMATION, 0.15),
        (r"^(go\s+ahead|proceed|continue|confirmed?|agreed?)[\s!.,?]*$", TrivialCategory.CONFIRMATION, 0.15),
        (r"^(sounds?\s+good|works?\s+for\s+me|that'?s?\s+fine)[\s!.,?]*$", TrivialCategory.CONFIRMATION, 0.15),

        # Thanks (low importance)
        (r"^(thanks?|thank\s+you|thx|ty|cheers|appreciated?)[\s!.,?]*$", TrivialCategory.THANKS, 0.1),
        ...
        # Acknowledgments
        (r"^(nice|great|awesome|cool|perfect|excellent|wonderful)[\s!.,?]*$", TrivialCategory.ACKNOWLEDGMENT, 0.15),
