...
Example - Minimal Configuration:
    from mindcore.svl import SVLPipeline, DefaultSourceConfig
...
class DefaultSourceConfig:
    ...
    default_params: list[ParamPattern] = field(default_factory=lambda: [ParamPattern.USER_ID])
    ...
    def get_table_name(self, topic: str) -> str:
        ...
        if self.naming_convention == NamingConvention.PLURAL:
            return self._to_plural(topic)
        if self.naming_convention == NamingConvention.SINGULAR:
...
class SmartTableSource(TableSource):
    ...
    def _build_dynamic_query(self, context: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        if where_clauses:
            ...
            query = (
                f"SELECT * FROM {safe_table} ORDER BY {self.fallback_order_by} LIMIT {safe_limit}"
            )
            available_params = {}
        else:
            raise ValueError(
