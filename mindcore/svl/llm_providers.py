...
class GeminiConfig(LLMProviderConfig):
    ...
    model: str = (
        "gemini-2.5-flash"  # gemini-2.5-pro, gemini-2.5-flash, gemini-3-pro, gemini-3-flash
    )
    ...
    def get_request_params(
        ...
        if include_reasoning:
            ...
            elif self.thinking_mode == ThinkingMode.DISABLED:
                params["generation_config"]["thinking_config"] = {"thinking_budget": 0}
    ...
    def get_gemini3_params(
        ...
        params: dict[str, Any] = {
            ...
            "generation_config": {
                ...
                "response_mime_type": "application/json",
                "response_schema": self._prepare_schema(json_schema),
                "thinking_config": {"thinking_level": thinking_level.value.upper()},
            },
        }
    ...
    def annotate_schema(self, schema: dict[str, Any]) -> dict[str, Any]:
        ...
        if "topics" in schema["properties"]:
            ...
            if self.feedback.effective_topics:
                preferred = ", ".join(
                    [f"'{t[0]}' ({t[1]:.0%})" for t in self.feedback.effective_topics[:5]]
                )
        ...
        if "categories" in schema["properties"]:
            desc = schema["properties"]["categories"].get(
