...
class ContextDecision:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ContextDecision:
        ...
        if isinstance(hcn, str):
            hcn = (
                HistoricalContextNeeded.TRUE
                if hcn.lower() == "true"
                else HistoricalContextNeeded.FALSE
            )
        return cls(
...
Respond with valid JSON only."""
        ...
        return {
            ...
            "properties": {
                ...
                "sentiment": {
                    ...
                    "enum": sentiments
                    if sentiments
                    else ["positive", "negative", "neutral", "mixed"],
                ...
                "emotional_classification": {
                    ...
                    "enum": emotional
                    if emotional
                    else ["neutral", "joy", "sadness", "anger", "fear"],
    ...
    def get_gemini_request(
        ...
        mode = (
            ...
            if thinking_mode in ["disabled", "dynamic", "fixed"]
            else ThinkingMode.DYNAMIC
        )
...
""" + "\n".join(guidance_lines)
    ...
    def get_effectiveness_enhanced_schema(
        ...
        if "properties" in schema and "topics" in schema["properties"]:
            ...
            if high_topics:
                effective_list = ", ".join(
                    [f"{t} ({s:.0%})" for t, s in list(high_topics.items())[:5]]
                )
