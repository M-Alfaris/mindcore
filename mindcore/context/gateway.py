...
if TYPE_CHECKING:
    ...
    from mindcore.svl.enforced_metadata import ContextDecision, EnforcedMetadata
...
- Dominant sentiment: {session.dominant_sentiment or 'neutral'}"""
    ...
    def _format_memories(self, memories: list[Memory]) -> str:
        ...
        for memory in memories:
            ...
            lines.append(f"- [{memory.memory_type}] {memory.content} {importance_marker}")
...
class ContextGateway:
            ...
            if result.current_session:
                ...
                if thread_id and result.memories:
                    result.memories = [
                        ...
                        or getattr(m, "thread_id", None) is None
                    ]
    ...
    def _store_response_memory(
        self, response_metadata: ResponseMetadata, response_text: str
    ) -> None:
    ...
    def invalidate_cache(
        ...
        for key in self._hot_cache:
            if (user_id and key.startswith(f"{user_id}:")) or (
                session_id and f":{session_id}:" in key
            ):
    ...
    def _prune_cache(self) -> None:
        ...
        sorted_keys = sorted(self._hot_cache.keys(), key=lambda k: self._hot_cache[k][1])
                ...
                for fetch_result in fetch_results:
                    ...
                    if fetch_result.data:
                        source_data[source_name].extend(
                            fetch_result.data
                            if isinstance(fetch_result.data, list)
