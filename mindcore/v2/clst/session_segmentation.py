...
Example:
    from mindcore.v2.clst import SessionManager
...
class SessionManager:
    ...
    def create_segment(
        ...
        if first_memory:
            segment.dominant_topic = (
                first_memory.topics[0] if first_memory.topics else None
            )
    ...
    def calculate_coherence(
        ...
        # Filter to session
        session_memories = [
            m for m in memories if getattr(m, "session_id", None) == session_id
        ]
    ...
    def get_session_segments(
        ...
        for sid, state in self._session_cache.items():
            if state.session_id.startswith(session_id) or session_id.startswith(
                state.session_id
            ):
    ...
    def _get_session_state(
        ...
        if session_memories:
            state.last_activity = max(
                getattr(m, "created_at", datetime.now(timezone.utc))
                for m in session_memories
            )
            state.recent_memories = session_memories[
                : self._policy.topic_shift_window_size
            ]
    ...
    def update_session_state(
        ...
        state.recent_memories = state.recent_memories[
            : self._policy.topic_shift_window_size
        ]
