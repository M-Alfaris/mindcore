...
class FederatedCLST:
    ...
    def store(
        ...
        if namespace.org_id != self.org_id:
            raise ValueError(f"Namespace org {namespace.org_id} doesn't match CLST org {self.org_id}")
    ...
    def search(
        ...
        if query.access_levels:
            storage_filter["access_level"] = {
                "$in": [level.value for level in query.access_levels]
            }
