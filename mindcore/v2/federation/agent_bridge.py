...
if TYPE_CHECKING:
    from .federated_clst import FederatedCLST, FederatedMemory, NamespacedQuery
...
class AgentMemoryBridge:
    ...
    def reinforce(
        ...
        if memory_id in self.local_memories:
            ...
            local.reinforcement_score = max(-1.0, min(1.0,
                local.reinforcement_score + signal * 0.5
            ))
            score = local.reinforcement_score

            # Auto-persist if threshold reached
            if (not local.persisted_to_clst and
                local.reinforcement_score > self.auto_persist_threshold):
    ...
    def get_local_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "persisted_count": sum(
                1 for m in self.local_memories.values() if m.persisted_to_clst
            ),
            "avg_reinforcement": (
                sum(m.reinforcement_score for m in self.local_memories.values()) /
                len(self.local_memories) if self.local_memories else 0
            ),
            "total_accesses": sum(
                m.access_count for m in self.local_memories.values()
            ),
        }
