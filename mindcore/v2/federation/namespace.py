...
class MemoryNamespace:
    ...
    def get_ancestors(self) -> list[MemoryNamespace]:
        ...
        # Team -> Department
        if self.team:
            ancestors.append(MemoryNamespace(
                org_id=self.org_id,
    ...
    def get_lineage(self) -> list[MemoryNamespace]:
        ...
        if self.department:
            lineage.append(MemoryNamespace(
                org_id=self.org_id,
                department=self.department,
            ))
...
class NamespaceHierarchy:
    ...
    def get_all_descendants(self, namespace: MemoryNamespace) -> list[MemoryNamespace]:
        ...
        if namespace.department is None:
            for dept, teams in self.departments.items():
                descendants.append(MemoryNamespace(
                    org_id=self.org_id,
                    department=dept,
                ))
                for team in teams:
        ...
        if from_namespace.department:
            ...
            for team in teams:
                if team != from_namespace.team:
                    visible.append(MemoryNamespace(
                        org_id=self.org_id,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            "org_id": self.org_id,
            "departments": {
                dept: list(teams) for dept, teams in self.departments.items()
            },
            "labels": {
                label: list(namespaces) for label, namespaces in self.labels.items()
            },
        }
    ...
    def from_dict(cls, data: dict[str, Any]) -> NamespaceHierarchy:
        ...
        return cls(
            org_id=data["org_id"],
            departments={
                dept: set(teams) for dept, teams in data.get("departments", {}).items()
            },
            labels={
                label: set(ns) for label, ns in data.get("labels", {}).items()
            },
        )
