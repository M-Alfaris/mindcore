...
class TestPolicyEnforcement:
    ...
    def test_reject_missing_required_fields(self, strict_gate):
        ...
        assert any(
            e.violation == PolicyViolation.MISSING_REQUIRED_FIELD
    ...
    def test_reject_invalid_memory_type(self, strict_gate):
        ...
        assert not result.success
        assert any(
            e.violation == PolicyViolation.INVALID_MEMORY_TYPE
            for e in result.errors
        )
    ...
    def test_reject_content_too_short(self, gate):
        ...
        assert not result.success
        assert any(
            e.violation == PolicyViolation.CONTENT_TOO_SHORT for e in result.errors
        )
    ...
    def test_retry_with_error_feedback(self, gate):
        ...
        def mock_llm(prompt):
            ...
            if call_count == 1:
                ...
                return '{"content": "Fixed content", "memory_type": "episodic", "topics": ["billing"]}'
...
class TestGatedStorage:
    ...
    def test_gated_mindcore_store(self, svl):
        ...
        from mindcore.v2.svl.gated_storage import GatedMindcore
