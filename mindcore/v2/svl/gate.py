...
Example:
    from mindcore.v2.svl import SVLGate, SharedVocabularyLayer
...
class SVLGate:
    ...
    def _canonicalize(self, data: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
        ...
        if "memory_type" in result:
            ...
            canonical = self._canonicalize_value(
                original, self._memory_type_map, "episodic"
            )
        ...
        if "sentiment" in result:
            ...
            canonical = self._canonicalize_value(
                original, self._sentiment_map, "neutral"
            )
            if canonical != original:
        ...
        if "access_level" in result:
            ...
            canonical = self._canonicalize_value(
                original, self._access_level_map, "private"
            )
            if canonical != original:
    ...
    def _validate(
        ...
        if "semantic_metadata" in data and data["semantic_metadata"]:
    ...
    def _retry_with_strategies(
        ...
        if self._retry_config.retry_with_simplified_schema:
            ...
            try:
                ...
                if all(
                    new_data.get(f)
                    for f in ["content", "memory_type", "user_id"]
                ):
                    return GateResult(
                        success=True,
                        decision=GateDecision.RETRY,
    ...
    def _create_retry_prompt(
        ...
        for error in errors:
            if error.allowed_values:
                value_hints.append(
                    f"  Valid {error.field}: {', '.join(error.allowed_values[:10])}"
                )
