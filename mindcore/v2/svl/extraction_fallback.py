...
if TYPE_CHECKING:
    ...
    from mindcore.v2.svl.enforced_metadata import EnforcedMetadata
...
class RuleBasedExtractor:
    ...
    def _detect_message_type(self, text: str) -> str:
        ...
        if text_lower.endswith("?") or any(w in text_lower for w in ["what", "how", "why", "when", "where", "who", "which"]):
            return "query"
        if any(w in text_lower for w in ["please", "can you", "could you", "would you", "i need", "i want"]):
            return "command"
        if any(w in text_lower for w in ["thanks", "thank you", "great", "good", "bad", "terrible"]):
            return "feedback"
    ...
    def _detect_sentiment(self, text: str) -> str:
        ...
        positive_words = ["great", "good", "excellent", "amazing", "love", "thanks", "helpful", "wonderful"]
        negative_words = ["bad", "terrible", "awful", "hate", "frustrated", "angry", "disappointed", "worst"]
    ...
    def extract_with_fallback(
        ...
        if ExtractionStrategy.RETRY_WITH_ERRORS not in skip and result.attempts:
            ...
            for retry in range(self.max_retries):
                attempt = self._try_retry_with_errors(
                    ...
                    user_id, session_id, last_error, retry + 1
                )
                result.attempts.append(attempt)
        ...
        if (
            ...
            attempt = self._try_batch_context_extraction(
                user_message, llm_call, agent_response,
                user_id, session_id, neighbor_messages
            )
            result.attempts.append(attempt)
    ...
    def _try_primary_extraction(
        ...
        try:
            ...
            return self._create_failure(
                ExtractionStrategy.PRIMARY, ExtractionFailureType.INVALID_JSON,
                f"JSON parse error: {e}", start
            )
        except ValueError as e:
            error_str = str(e)
            if "validation failed" in error_str.lower():
                return self._create_failure(
                    ExtractionStrategy.PRIMARY, ExtractionFailureType.INVALID_VOCABULARY,
                    error_str, start
                )
            return self._create_failure(
                ExtractionStrategy.PRIMARY, ExtractionFailureType.UNKNOWN,
                error_str, start
            )
